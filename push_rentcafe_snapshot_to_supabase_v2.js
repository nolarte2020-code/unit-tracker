// push_rentcafe_snapshot_to_supabase_v2.js
// Universal RentCafe adapter: DOM-first (avail units) + floorplan fallback + optional JSON sniffer
...
function bedsHintFromUrl(url) {
  // Very light heuristic for reporting/meta only (doesn't need to be perfect)
  const u = String(url || "").toLowerCase();
  if (u.includes("studio")) return 0;
  if (u.includes("single")) return 0; // many sites use "single" like studio/efficiency
...
function parseDateAvailableToISO(raw) {
  // Accepts "1/2/2026", "01/02/2026", "1-2-2026"
...
async function extractAvailableUnitsFromDom(page, pageUrl) {
  // Try common containers; not all themes use #availApts
  await Promise.race([
  ...
  const rawBlocks = await page.evaluate(() => {
    ...
    // Generic candidates: list rows/cards/items that often contain unit info
    ...
    const blocks = candidates
      ...
      .filter((t) => /\$[0-9,]+/.test(t) || /\bAvailable\b/i.test(t) || /\bApartment\b/i.test(t));
  ...
  for (const block of rawBlocks) {
    ...
    // "Available Now"
    // "Date Available: 1/2/2026"
    const dateAvailRaw = firstMatch(block, /Date\s*Available\s*:\s*([0-9]{1,2}[\/\-][0-9]{1,2}[\/\-][0-9]{4})/i);
    const dateAvailIso = dateAvailRaw ? parseDateAvailableToISO(dateAvailRaw) : null;

    const availableText =
      dateAvailIso ||
      firstMatch(block, /(Available\s+Now)/i) ||
      firstMatch(block, /(Available\s+(?:[A-Za-z]{3,9}\s+[0-9]{1,2}(?:st|nd|rd|th)?))/i) ||
      (/\bAvailable\b/i.test(block) ? "Available" : null);

    // If it doesn't look like a unit row, skip
    if (!unitNum && !priceText && !availableText) continue;
    ...
    // unit_id should be stable; include bedsHint only in meta (not in id)
    const unitId = unitNum ? `unit:${unitNum}` : `unit:unknown:${Buffer.from(key).toString("base64").slice(0, 24)}`;

    results.push({
      unit_id: unitId,
      unit_number: unitNum || null,
      available_on: availableText || null, // ISO if date available, otherwise "Available Now"/"Available"
...
async function fetchRentCafePage(url) {
  ...
  try {
    ...
    // Tier 1: DOM unit-level extraction
    const domUnits = await extractAvailableUnitsFromDom(page, url);

    // Tier 2: fallback parse from HTML snapshot
...
// ====== SightMap-style: fetch -> extractUnits -> upsertSnapshot ======
async function fetchRentCafePayload() {
  const urls = normalizeUrlList();
  const pages = [];

  for (const u of urls) {
    pages.push(await fetchRentCafePage(u));
  }
...
function extractUnits(payload) {
  ...
  for (const p of pages) {
    ...
    // Tier 2: HTML floorplan fallback
    const html = p?.html || "";
...
// Clean exit wrapper (prevents Windows async handle weirdness)
main()
