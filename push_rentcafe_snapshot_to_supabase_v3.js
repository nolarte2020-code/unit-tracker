// push_rentcafe_snapshot_to_supabase_v3.js
// Universal RentCafe adapter: DOM-first (real unit rows only) + floorplan fallback + optional JSON sniffer
...
async function extractAvailableUnitsFromDom(page, pageUrl) {
  ...
  const rawBlocks = await page.evaluate(() => {
    ...
    const blocks = candidates
      ...
      .filter(
        (t) =>
          /\bApartment\b/i.test(t) ||
          /\bUnit\b/i.test(t) ||
          /#\s*[A-Za-z0-9-]{2,20}/.test(t) ||
          /\$[0-9,]+/.test(t) ||
          /\bDate Available\b/i.test(t)
      );
  ...
  for (const block of rawBlocks) {
    ...
    // Skip marketing/non-unit availability content
    if (
    ...
    // STRICT REQUIREMENT: only count real unit rows
    if (!unitNum) continue;
    ...
    // Optional: require either price or availability text
    ...
    results.push({
      ...
      unit_number: unitNum,
      available_on: availableText || null, // ISO date if present
