// push_rentcafe_snapshot_to_supabase.js (Universal RentCafe adapter: DOM-first + floorplan fallback + optional sniffer)
...
const RENTCAFE_URLS = process.env.RENTCAFE_URLS; // comma-separated

// Optional: save JSON responses while debugging
...
// ====== DOM-FIRST EXTRACTION (units list under #availApts) ======
// Works great for pages like /floorplans/one-bedroom#availApts
async function extractAvailableUnitsFromDom(page) {
  // Wait briefly for the availability section to exist (if it ever will)
  await page
    .waitForSelector("#availApts", { timeout: 6000 })
    .catch(() => null);

  // Pull a DOM snapshot and parse text blocks.
  // We intentionally keep this "generic" because different RentCafe themes use different classes.
  const rawBlocks = await page.evaluate(() => {
    const root = document.querySelector("#availApts") || document.body;

    // Candidates: list items, cards, rows, etc. (generic heuristics)
    const candidates = Array.from(
      root.querySelectorAll("li, .row, .card, .available, [class*='avail'], [class*='unit']")
    );

    // Keep blocks with some price-like text or "Available"
  ...
  for (const b of rawBlocks) {
    // Example patterns we commonly see:
    // "Apartment #221 Available Now Starting at: $2,345"
    // "221 Available Now $2,345"
    // We'll be flexible.

    const unitNum =
      firstMatch(b, /Apartment\s*#?\s*([A-Za-z0-9-]+)/i) ||
      firstMatch(b, /\b#\s*([A-Za-z0-9-]{2,10})\b/) ||
    ...
    const availableText =
      firstMatch(b, /(Available\s+(?:Now|[A-Za-z]{3,9}\s+[0-9]{1,2}(?:st|nd|rd|th)?))/i) ||
      (/\bAvailable\b/i.test(b) ? "Available" : null);

    // If it doesn't look like a unit row, skip
    if (!unitNum && !priceText) continue;
    ...
    results.push({
      unit_id: unitNum ? `unit:${unitNum}` : `unit:unknown:${key}`,
      unit_number: unitNum || null,
      available_on: availableText || null, // keep as text for now (consistent with your RentCafe floorplan logic)
...
// ====== FLOORPLAN FALLBACK (your existing parser) ======
function parseFloorplansFromHtml(html) {
  ...
  for (const b of blocks) {
    ...
    const availableOnText =
      firstMatch(text, /(Available\s+(?:Now|[A-Za-z]{3,9}\s+[0-9]{1,2}(?:st|nd|rd|th)?))/i);
    ...
    results.push({
      ...
      available_on: availableOnText || null,
      price: null,
      floor_plan_id: null,
      meta: {
        beds,
...
async function fetchRentCafePage(url) {
  ...
  try {
    ...
    // Tier 1: try #availApts (units list)
    const domUnits = await extractAvailableUnitsFromDom(page);

    const html = await page.content();
    return { html, domUnits };
  } finally {
...
// ====== SightMap-style pattern: fetch -> extractUnits -> upsertSnapshot ======
...
function extractUnits(payload) {
  ...
  for (const p of pages) {
    // Tier 1: DOM unit-level extraction if present
    ...
    // Tier 2: fallback floorplan parse from HTML
    const html = p?.html || "";
...
async function main() {
  ...
  if (units.length === 0) {
    console.log("⚠️ Extracted 0 units.");
